'use client'
import axios from "axios";
import { useEffect, useState } from "react";
function InvStat() {
  const product_url = process.env.NEXT_PUBLIC_DB_API_PRODUCT;

  const[stat,setStat] = useState([])
  const [error,setError] = useState(null)
  
  useEffect(()=>{
    const get_type_count = async ()=>{
      
      try{
        const response = await axios.get(`${product_url}countByType`,{withCredentials:true})
        if(response.data){
          setStat(response.data)
        }
  
      } catch(error){
         setError(error.message)
      }
     }
     get_type_count();
   
  },[])
 
  return (  
    <div className="m-4 px-2 w-full h-auto">
      <div className="flex flex-col items-center px-3 py-3 shadow-lg rounded-xl bg-gradient-to-r from-slate-500 to-neutral-400 w-full">
        
        {/* Title */}
        <h4 className="text-lg font-semibold text-gray-900 mb-3">
          Inventory Status
        </h4>

        {/* Stats Row */}
        { error ? ( <p  className="text-red-200 text-2xl"> {error}</p>):

        (<div className="flex flex-row space-x-1 overflow-x-auto w-full justify-center">
          {stat.map((item, idx) => (
            <div 
              key={idx} 
              className="bg-white shadow-sm rounded-md w-20 px-1 py-1 text-center shrink-0"
            >
              <p className="text-[9px] font-semibold text-gray-800 truncate border border-solid bg-green-600 rounded-2xl">
                {item._id}
              </p>
              <p className="text-[8px] font-bold text-gray-800 mt-1 bg-gray-100 rounded px-1 py-0.5">
                Qty: {item.totalCount}
              </p>
            </div>
          ))}
        </div>)
        }
      </div>
    </div>
  );
}

export default InvStat;
